...
/*
	...
	Contient : Fonctions relatives à la BDD - Adapté pour les serveurs hébergés aux USA - intervalle à adapter
...
function AffichageCommentaires($id){
	...
	$askForComments = $base->prepare("SELECT id, auteur, commentaire, DATE_FORMAT(DATE_ADD(date_commentaire, INTERVAL 6 HOUR), 'le %d/%m/%Y à %H:%i') AS datewrote FROM commentaires WHERE id_billet = :id");
...
function AffichageBillet($id){
	...
	$askForPosts = $base->prepare("SELECT titre, auteur, contenu, id, DATE_FORMAT(DATE_ADD(date_creation, INTERVAL 6 HOUR), 'le %d/%m/%Y à %H:%i') AS datewrote, image, available FROM billets WHERE id = :id");
...
function AffichageCommentairesGeneral(){
	...
	//Requête d'accès aux commentaires selon billet
	$askForComments = $base->query("SELECT id, auteur, commentaire, DATE_FORMAT(DATE_ADD(date_commentaire, INTERVAL 6 HOUR), 'le %d/%m/%Y à %H:%i') AS datewrote FROM commentaires ORDER BY id DESC");
...
function AffichagePostsGeneral(){
	...
	//Requête d'accès aux commentaires selon billet
	$askForPosts = $base->query("SELECT auteur, id, titre, contenu, DATE_FORMAT(DATE_ADD(date_creation, INTERVAL 6 HOUR), 'le %d/%m/%Y à %H:%i') AS datecomment, available FROM billets ORDER BY id DESC");
...
function AffichageComment($id){
	...
	//Requête d'accès au billet demandé
	$askForComments = $base->prepare("SELECT id, auteur, commentaire, DATE_FORMAT(DATE_ADD(date_commentaire, INTERVAL 6 HOUR), 'le %d/%m/%Y à %H:%i') AS datewrote FROM commentaires WHERE id = :id");
...
function setRank($id, $rank, $banchat)
{
	global $base;
...
function banUserFromChat($banchat)
{
	global $base;

	$userToSet = $base->prepare("UPDATE membres SET banchat = :banchat WHERE id = :id");
	$userToSet->execute(array("banchat"=>$banchat,
							  "id"=>$id));
}

function postMessageOnChat($userId, $message)
{
	global $base;

	$messageToSend = $base->prepare("INSERT INTO blogchat(user_id,date,message) VALUES (:userId,NOW(),:message)");
	$messageToSend->execute(array("userId"=>$userId,
								  "message"=>$message));
}

function getMessageOnChat()
{
	//Définit les variables de session
	global $base;
	$messages = $base->query("SELECT * FROM blogchat LIMIT 0,20");
